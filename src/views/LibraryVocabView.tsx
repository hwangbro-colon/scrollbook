...
export function LibraryVocabView() {
  ...
  const vocabAsync = useSimulatedAsync({ delayMs: 500 });
  const suggestedWords = (book?.vocabCandidates ?? []).filter((c) => !vocabList.some((v) => v.word === c.word));
  ...
  return (
    <ScreenScroll>
      ...
      <p className="mb-4 mt-0.5 text-xs text-[var(--color-ink-soft)]">단어 저장 · 플래시카드 복습</p>
      ...
      {suggestedWords.length > 0 && (
        <div className="mt-3 flex flex-col gap-2">
          <p className="text-[10.5px] font-bold text-[var(--color-ink-soft)]">{book?.title}에서 추천하는 단어</p>
